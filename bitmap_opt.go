...
func Masked(bm *Bitmap, mask uint64) *Bitmap {
	return maskedInto(bm, mask, NewBitmap())
}
...
func MaskedToBuf(bm *Bitmap, mask uint64, buf []byte) *Bitmap {
	return maskedInto(bm, mask, NewBitmapToBuf(buf))
}
...
// (entries slice + two scratch buffers vs one) and the sort cost outweigh
...
func maskedInto(bm *Bitmap, mask uint64, b *Bitmap) *Bitmap {
	if bm == nil {
		return b
	}
	an := bm.keys.numKeys()
...
// Equivalent to !And(a, b).IsEmpty() but short-circuits at the first common
// element without allocating a result bitmap.
func Intersects(a, b *Bitmap) bool {
	...
	// Start bi at the first b key >= a's first key.
	bi := b.keys.search(a.keys.key(0))
